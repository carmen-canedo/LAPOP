...
THIS IS RIGHT
=======
>>>>>>> 0c308015a932150582373448afc53a48d5faa38f
...
argentina_wide <- try(argentina_long)
```

```{r}
argentina_and_belize <- list(argentina_long, belize_long)
argentina_and_belize <- map_df(argentina_and_belize, lengthen)

argentina_and_belize <- argentina_and_belize %>%
  spread(key = questions, value = answer)
```

```{r}
argentina_spread <- argentina_long %>% select(-column_name) %>% spread(questions, answer)
```
